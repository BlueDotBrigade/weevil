namespace BlueDotBrigade.Weevil.Analysis.Timeline
{
	...
	public class DetectFirstAnalyzerTests
	{
		...
		// DetectFirstAnalyzer flags the first record where each unique value appears.
			...
			string label)
		{
			...
			results.FlaggedRecords.Should().Be(expectedIndices.Length,
				because: $"[{label}] pattern '{pattern}' should report {expectedIndices.Length} unique-value first occurrence(s)");
		}
			...
			int[] actualIndices)
		{
			...
			{
				$"[{label}] first-occurrence flag mismatch.",
				$"  Pattern : {pattern}",
				$"  Expected: {expected}",
		...
		{
			// Same noisy sequence as the falling/rising tests. DetectFirstAnalyzer flags the first
			...
			GetFlaggedIndices(records).Should().Equal(0, 2, 4, 7);
