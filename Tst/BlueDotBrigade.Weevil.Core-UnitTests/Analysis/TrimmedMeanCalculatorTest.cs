namespace BlueDotBrigade.Weevil.Statistics
{
	...
	public class TrimmedMeanCalculatorTest
	{
		...
		public void Calculate_WithTooSmallList_ReturnsNull()
		{
			...
			var timestamps = new List<DateTime> { DateTime.Now, DateTime.Now };
			...
			// Assert
			Assert.AreEqual("TrimmedMean", result.Key);
			Assert.IsNull(result.Value); // Should return null when trim count is too large
		}
		...
		public void Calculate_UsesArrayLength_NotOriginalCount()
		{
			...
			// Mean = 15/3 = 5.0
			Assert.AreEqual(5.0, (double)result.Value, 0.001);
		}
	}
}
