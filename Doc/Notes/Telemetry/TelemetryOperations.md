...
1. Confirm environment credentials are present when SQL authentication is required.
2. Verify that opening a file starts a session and opening a different file ends the previous session.
3. Verify that closing the application ends the current session once.
4. Verify that a credentials-blank environment produces no upload attempts.
5. Verify that an invalid connection string does not crash the application.
6. Verify that the database receives one row per ended session.
7. Verify that the inserted row contains `Application`, `Source`, `Version`, `IsDebugging`, `SessionStartUtc`, `SessionEndUtc`, `SessionActiveMinutes`, `LogFileSizeBytes`, `InstalledRamMb`, `FilterExecutionCount`, `GraphOpenCount`, `DashboardOpenCount`, and `SchemaVersion`.
