...
| **Next** | **Silence Detection** | Gap detection, missing event detection, heartbeat loss detection, no-log detection | Detects when expected records stop appearing. Natural extension of elapsed-time analysis. |
...
| Roadmap Fit | Analysis Operation | Aliases / Related Terms | Purpose / Value |
| --- | --- | --- | --- |
| **Implemented** | **State Transition Detection** | Detect Data Transitions, change detection, value transition detection, state change detection | Flags records where a captured value changes from the previous captured value. |
...
| **Implemented** | **Elapsed Time Threshold Detection** | Measure Elapsed Time, gap threshold detection, pause detection | Flags records where elapsed time between records exceeds a threshold. Useful for pauses, hangs, startup delays, and blocked background work. |
