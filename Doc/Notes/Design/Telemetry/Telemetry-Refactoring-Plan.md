...
GUID and the bigint are not both needed; that was my over-complication. The real question is: what makes a re-upload idempotent after a force-kill?

A bigint IDENTITY is assigned by the server at insert time. The client never knows it before uploading, so on a re-upload the row just gets a fresh id — the DB has no way to recognize "I already stored this session." Crash-proof dedup therefore requires a key the client generates before upload and the DB enforces as UNIQUE. A GUID is exactly that, generated once at session start (it's also already the outbox filename).

So the GUID isn't extra baggage sitting next to a bigint — it's the one thing doing the dedup work. The fix for your "I'd prefer not to keep the GUID + bigint" instinct is: drop the bigint, not the GUID. Use the GUID as the sole PK (which is what the plan doc and the current code already do). One key, no identity column, no child-FK-under-DENY-SELECT risk — and it's the simplest option.

The only way to drop the GUID entirely is to give up the hard guarantee and accept rare duplicate rows (deduped at report time).
